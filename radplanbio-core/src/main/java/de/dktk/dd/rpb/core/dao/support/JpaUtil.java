 ...
 * @author tomas@skripcak.net - enhancements
 * @since 01 Apr 2013
 */
public class JpaUtil {
    ...
    private static boolean isManuallyAssigned(Method method) {
        // Is Id method and is not GeneratedValue
        if (method.getAnnotation(Id.class) != null) {
            return method.getAnnotation(GeneratedValue.class) == null;
        }
        // Is not Id method and is not EmbeddedId
    ...
    public static <E> Predicate stringPredicate(Expression<String> path, Object attrValue, SearchParameters sp, CriteriaBuilder builder) {
        ...
        if (sp.isCaseInsensitive()) {
            processedPath = builder.lower(path);
        ...
        switch (sp.getSearchMode()) {
            case EQUALS:
                return builder.equal(processedPath, attrValue);
            case ENDING_LIKE:
                return builder.like(processedPath, "%" + attrValue);
            case STARTING_LIKE:
                return builder.like(processedPath, attrValue + "%");
            case ANYWHERE:
                return builder.like(processedPath, "%" + attrValue + "%");
            case LIKE:
                return builder.like(processedPath, (String) attrValue); // assume user provide the wild cards
            default:
