...
public class StagedSubjectPacsResultBuilder {
    ...
    private void migrateEnrollmentDateFromStudySubjectToSubject(Subject subject, StudySubject studysubject, SimpleDateFormat sdf) {
        if (studysubject.getEnrollmentDate() != null) {
            try {
                subject.setEnrollmentDate(
                        DatatypeFactory.newInstance().newXMLGregorianCalendar(studysubject.getEnrollmentDate().toString())
                );
    ...
    private StudySubject getFirstMatchingStudySubjectByStudySubjectId(String studySubjectId) {
        for (StudySubject studySubject : queriedStudySubjects) {
            if (studySubjectId.equals(studySubject.getStudySubjectId())) {
                return studySubject;
            }
        }
        return null;
