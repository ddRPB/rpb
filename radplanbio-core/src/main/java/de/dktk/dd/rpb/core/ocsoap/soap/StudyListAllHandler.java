...
public class StudyListAllHandler implements SOAPHandler<SOAPMessageContext> {
	 ...
	 */
	public StudyListAllHandler() throws ParserConfigurationException {
	 ...
	 * @throws ParserConfigurationException 
	 */
	public StudyListAllHandler(ArrayList<String> messages) throws ParserConfigurationException {
		super();
		...
		if (n.getLength() > 0) {
			...
			for (int i = 0; i < n.getLength(); ++i) {
				...
				if (n.item(i).getNodeType() == Node.ELEMENT_NODE) {
					Node node = n.item(i);
					d.renameNode(node, STUDIES_TARGET_NS, node.getNodeName());
				}
			}
		}
	}
