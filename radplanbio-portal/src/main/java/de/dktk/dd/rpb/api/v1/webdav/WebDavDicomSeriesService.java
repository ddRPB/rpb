 ...
 */
@Component
@Path("/v1/webdav/studies/{studyIdentifier}/sites/{siteIdentifier}/subjects/{studySubjectIdentifier}/events/{eventIdentifier}/dcmstudies/{dicomStudyIdentifier}/dcmseries")
public class WebDavDicomSeriesService extends BaseService {

    //region Finals

    private static final Logger log = LoggerFactory.getLogger(WebDavDicomSeriesService.class);
    ...
    public final javax.ws.rs.core.Response propfind(
        ...
        if (WebDavUtils.isWindowsFileSystemIdentifier(studyIdentifier, siteIdentifier, eventIdentifier, studySubjectIdentifier, dicomStudyIdentifier)) {
    ...
    public final javax.ws.rs.core.Response profind(
        ...
        // Workaround for windows native client that tries to lookup for things that do not exists in RPB
        if (WebDavUtils.isWindowsFileSystemIdentifier(studyIdentifier,eventIdentifier, siteIdentifier, studySubjectIdentifier,  dicomStudyIdentifier, dicomSeriesIdentifier)) {
            return javax.ws.rs.core.Response.status(404).build();
        }
        ...
        if (studyIdentifier != null && !studyIdentifier.isEmpty() &&
            ...
            log.error("WebDAV: Requested study/site/subject/event/dcmstudy/dcmseries identifier is null or empty.");
